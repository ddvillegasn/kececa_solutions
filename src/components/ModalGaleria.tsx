...
export default function ModalGaleria({ isOpen, onClose, images, title, description }: ModalGaleriaProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full p-0 relative animate-fade-in flex flex-col" style={{ maxHeight: '90vh' }}>
        <div className="flex-shrink-0 p-4 border-b border-gray-200 dark:border-gray-800 relative">
        ...
        <div className="overflow-y-auto px-6 py-4" style={{ maxHeight: '70vh' }}>
          ...
          <div className="flex flex-col items-center mb-6">
            <div className="relative flex items-center justify-center mb-2 w-full max-w-md mx-auto">
              <button onClick={prev} className="absolute left-0 px-3 py-2 text-2xl text-gray-600 hover:text-blue-600 z-10">❮</button>
              <img src={images[current]} alt={title + ' galería'} className="max-h-60 rounded-xl mx-auto object-contain bg-white shadow w-full" style={{maxWidth:'100%'}} />
              <button onClick={next} className="absolute right-0 px-3 py-2 text-2xl text-gray-600 hover:text-blue-600 z-10">❯</button>
            </div>
          ...
          <div className="prose prose-blue dark:prose-invert max-w-lg mb-2 text-left mx-auto text-base leading-relaxed" style={{whiteSpace: 'normal'}}>
            <ReactMarkdown
              components={{
